...
class FelicaCodecTest {
    ...
    @Test fun genericImageAlignsSystemCodeBlock() {
        ...
        assertEquals(
            ...
            HexCodec.encode(requireNotNull(genericImage.read(0x82)))
        )
        assertEquals(
            "88B40000000000000000000000000000",
            HexCodec.encode(requireNotNull(aimeImage.read(0x85)))
        )
        assertEquals(
            "40000000000000000000000000000000",
            HexCodec.encode(requireNotNull(genericImage.read(0x85)))
        )
    }
